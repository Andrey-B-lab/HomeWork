resource "null_resource" "wait_for_mysql_instance" {
  provisioner "local-exec" {
    ...
    while [ -z "$(aws ec2 describe-instances --filters "Name=tag:Name,Values=MySQL-Instance" "Name=instance-state-name,Values=running")" ]; do
      echo "MySQL instance is not ready yet, waiting..."
      sleep 10
    done
    echo "MySQL instance is now running with an assigned public IP!"
    EOT
  }
